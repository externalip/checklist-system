                ...
                <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                    ...
                    <thead style="background-color: #3c5393" class="text-xs text-white">
                        <tr>
                            ...
                            <th class="border-r-[2px] border-b-[2px] border-blue-900 px-1 py-1" v-for="(section, sectionIndex) in JSON.parse(row.form_data)?.form_content" :key="sectionIndex" :colspan="getQuestionCount(section)">
                        ...
                        <tr>
                            <template v-for="(section, sectionIndex) in JSON.parse(row.form_data)?.form_content">
                                <th v-for="(question, questionIndex) in section.section_content" :key="questionIndex" class="whitespace-nowrap border-r-[2px] border-blue-900 p-2">
                                    {{ question.label }}
                                </th>
                            </template>
                    ...
                    <tbody v-for="response_field in data.data">
                        <tr v-if="(response_field.form_id === selectedOption &&
                                ...
                                (response_field.form_id === selectedOption &&
                                    response_field.operator_employee_id ===
                                    selectedUser && selectedStatus === 'default')" class="hover:bg-gray-100 duration-200 bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <template v-for="(section, sectionIndex) in JSON.parse(response_field.form_data)?.form_content">
                                <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white" v-for="(question, questionIndex) in section.section_content" :key="questionIndex">
