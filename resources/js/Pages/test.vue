    ...
    <AppLayout :title="formData.form_name">
        <div class="5s lg:mx-[25%]">
            <h3 class="text-center">
                <marquee> Control No: {{ JSON.parse(formData.form_data).control_no }}</marquee>
            </h3>
            <form @submit.prevent="submit()" method="post" id="1">
                ...
                <div v-for="(section, sectionIndex) in JSON.parse(formData.form_data)?.form_content" :key="sectionIndex">
                    <section id="form-section" class="p-10 mt-5 mb-5 border-2 rounded-lg">
                        <!-- {{ section }} -->
                        <h2>{{ section.section_name }}</h2>
                        <!-- inside section get the question  -->
                        <div v-for="(question, questionNumber) in section.section_content" :key="questionNumber"
                            class="p-5 mt-5 mb-5 border-2 rounded-lg">

                            <h3 class="text-lg">Question: {{ question.label }}<span v-if="question.required"
                                    style="color: red;" class="text-sm"> * </span></h3>
                                    ...
                                    <li v-for="(option, optionIndex) in question.options" :key="optionIndex"
                                        ...
                                        <div class="flex items-center pl-3">
                                            <input
                                                ...
                                                :required="question.required" />
                                            <label
                                                :for="'production-checksheet-radio-symbol-' + sectionIndex + '-' + questionNumber + '-' + optionIndex"
                                    ...
                                    <li v-for="(option, optionIndex) in question.options" :key="optionIndex"
                                        ...
                                        <div class="flex items-center pl-3">
                                            <input
                                                ...
                                                :required="question.required" />
                                            <label
                                                :for="'production-checksheet-radio-' + sectionIndex + '-' + questionNumber + '-' + optionIndex"
                                                :style="{ color: option.color }"
                                                class="w-full py-3 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                                {{ option.label }}
                                            </label>
                ...
                <section id="form-section" class="p-10 mt-5 mb-5 border-2 rounded-lg">
                    ...
                    <div id="question" class="border-2 mb-3 py-5 px-10 md:px-10 md:py-5 rounded-md md:rounded-md">
                        ...
                        <label for="message" class="block mb-2 text-sm font-medium dark:text-white">Please type possible
                            problems regarding with the machine.</label>
