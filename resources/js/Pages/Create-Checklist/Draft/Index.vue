<template>
    <AppLayout :title="formData.form_name">
            ...
            <form @submit.prevent="submit()" method="post" id="1">
                <section id="form-section" class="p-10 mt-5 mb-5 border-2 rounded-lg">
                    ...
                    <div id="model-identification" class="grid grid-cols-1">
                        ...
                        <div id="model-identification"
                            ...
                            <select id="models"
                                ...
                                required v-model="form.fieldAnswers['Model Name']">
                    ...
                    <div id="model-identification" class="grid lg:grid-cols-2 lg:gap-3">
                        <div id="Lot Number" class="border-2 mb-3 py-5 px-10 md:px-10 md:py-5 rounded-md md:rounded-md">
                            ...
                            <div class="">
                                ...
                                <input type="text" id="ltnum" placeholder="Ex. 5/46"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    v-model="form.fieldAnswers['Lot Number']" required />
                            </div>
                        </div>
                        <div id="Kit Number" class="border-2 mb-3 py-5 px-10 md:px-10 md:py-5 rounded-md md:rounded-md">
                            ...
                            <div class="">
                                ...
                                <input type="text" id="ltnum" placeholder="Ex. 36132-001"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    v-model="form.fieldAnswers['Kit Number']" required />
                            </div>
                        </div>
                    </div>
                </section>
                <div v-for="(section, sectionIndex) in JSON.parse(
                    formData.form_data
                )?.form_content" :key="sectionIndex">
                    <section id="form-section" class="p-10 mt-5 mb-5 border-2 rounded-lg">
                        <!-- {{ section }} -->
                        ...
                        <div v-for="(
                            ...
                            ) in section.section_content" :key="questionNumber"
                            class="p-5 mt-5 mb-5 border-2 rounded-lg">
                            <h3 class="text-lg">
                                Question: {{ question.label }}
                                <span v-if="question.required" style="color: red" class="text-sm">
                                    *
                                </span>
                            </h3>
                            <h5 class="text-lg bold" v-if="question.instruction != null">
                                ...
                                <ul
                                    ...
                                    <li v-for="(
                                        ...
                                        <div class="flex items-center pl-3">
                                            <input :id="'production-checksheet-radio-symbol-' +
                                                sectionIndex +
                                                '-' +
                                                questionNumber +
                                                '-' +
                                                optionIndex
                                                " :name="'production-checksheet-radio-symbol' +
        sectionIndex +
        '-' +
        questionNumber
        " v-model="form.fieldAnswers[
        question.label
        ]
        " :value="option.label" type="radio"
                                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500"
                                                :required="question.required" />
                                            <label :for="'production-checksheet-radio-symbol-' +
                                                sectionIndex +
                                                '-' +
                                                questionNumber +
                                                '-' +
                            ...
                            <template v-if="question.type === 'radio'">
                                <ul
                                    ...
                                    <li v-for="(
                                        ...
                                        class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                        <div class="flex items-center pl-3">
                                            <input :id="'production-checksheet-radio-' +
                                                sectionIndex +
                                                '-' +
                                                questionNumber +
                                                '-' +
        ...
        " :value="option.label" type="radio"
                                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500"
                                                :required="question.required" />
                                            <label :for="'production-checksheet-radio-' +
                                                sectionIndex +
                                                '-' +
                                                questionNumber +
                                                '-' +
                            ...
                            <template v-if="question.type === 'checkbox'">
                                <div v-for="(
                                        option, optionIndex
                                    ) in question.options" :key="optionIndex" class="flex items-center mb-1">
                                    <input :id="'checkbox' + optionIndex" type="checkbox" :value="option.label" :checked="form.fieldAnswers[question.label] &&
                                        form.fieldAnswers[
                                            question.label
        ...
        "
                                    ...
                                    <label :for="'checkbox' + optionIndex"
                                        ...
                                        :required="question.required">{{ option.label }}</label>
                                </div>
                            </template>

                            <template v-if="question.type === 'dropdown'">
                                <!-- Dropdown (select) rendering -->
                                <select :id="'production-checksheet-select-' +
                                    sectionIndex +
                                    '-' +
                                    questionNumber +
                                    '-' +
                                    optionIndex
                                    " :name="'production-checksheet-select-' +
        sectionIndex +
        '-' +
        questionNumber +
        '-' +
        ...
        " v-model="form.fieldAnswers[question.label]"
                                    ...
                                    <option v-for="(
                                            option, optionName
                                        ) in question.options" :key="optionName" :value="option.label">
                                        {{ option.label }}
                                    </option>
                                </select>
                            </template>
                ...
                <section id="form-section" class="p-10 mt-5 mb-5 border-2 rounded-lg">
                    ...
                    <div id="question" class="border-2 mb-3 py-5 px-10 md:px-10 md:py-5 rounded-md md:rounded-md">
                        ...
                        <label for="message" class="block mb-2 text-sm font-medium dark:text-white">Please type possible
                            problems regarding with the
                            machine.
                            <span class="text-sm" style="color: red"> * </span>
                        </label>
                        <textarea v-model="form.fieldAnswers['Remarks on the Model']" id="message" rows="4"
