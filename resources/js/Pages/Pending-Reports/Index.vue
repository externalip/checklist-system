...
const submit = async (status, id) => {
    if (status === 'OK' || status === 'Rejected') {
        const { isConfirmed } = await Swal.fire({
            ...
            confirmButtonText: status === 'OK' ? 'Yes, Approve it!' : 'Yes, Reject it!',
        ...
        try {
            const response = await axios.put(route('Pending-Reports.update', {status, id}));
                            ...
                            <div :id="'accordion-flush-body-' + index.toString()" class="hidden border-2 border-gray-100" :aria-labelledby="'accordion-flush-heading-' + index.toString()">
                                ...
                                <div v-if="$page.props.auth.employee.role_id != 1 " class="pb-5">
                                    ...
                                    <div id="pending-sign-btn" class="px-5 p-2 w-full flex justify-end">
                                        ...
                                        <button type="button" href="#" @click="submit('Rejected',row.id)" class="focus:outline-none text-white bg-[--red] hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Reject
                                        </button>

                                        <!-- </router-link> -->

                                        <!-- Approve Button -->
                                        <button type="button" href="#" @click="submit('OK',row.id)" class="text-white bg-[--blue] hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Approve
                                        </button>
